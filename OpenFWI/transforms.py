...
def minmax_normalize(vid, vmin, vmax, scale=2):
    vid = vid - vmin
    vid = vid/(vmax - vmin)   # 0-1
    return (vid - 0.5) * 2 if scale == 2 else vid  # -1 - 1
...
class PCD(object):
    ...
    def __call__(self, data):
        ...
        data = data - np.tile(feat_mean, (data.shape[0], 1))
...
class StackPCD(object):
    ...
    def __call__(self, data):
        ...
        for sample in data:
            ...
            sample = sample - np.tile(feat_mean, (sample.shape[0], 1))
